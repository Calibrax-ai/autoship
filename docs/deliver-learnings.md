...
Five probes, `probe 0.1` through `probe 0.5`. The headline: **deliver's grooming-and-build pattern is proven across four change shapes** — Bug, Feature, Refactor, and UI build. A human-approved spec turns into a reviewed code change through a draft pull request without losing control of scope or evidence.
...
- `pre-groomer` writes the spec
- `spec-reviewer` judges it
- operator sees the verdict
- regroom runs only on reject

Across four real issues, 3/4 specs reached `ready-for-oracle` and average regroom count was 1.0.
...
FRD-143 exposed a specific tendency: when a Feature touched several surfaces, the pre-groomer defaulted toward a unified spec rather than stepping back and asking whether the issue should decompose.

The resulting spec was defensible, but not obviously the simplest reasonable shape.

**Implication:** decomposition needs to become procedural, not merely allowed by the schema. A scope-check step should happen before the spec is written.
...
## Build validation — trustworthy spec drives clean execution on one clean Bug
...
The implementation matched the spec's Skeleton Position closely and only made one explicit judgment call for a flagged ambiguity.

**Implication:** a reviewer-approved spec can be a real execution contract, not just a planning artifact, at least for a narrow bug-shaped issue.
...
6. Feature specs touching schema or migration surfaces must include the triggered conditional sections.
...
**Stage 1 (oracle).** One fresh `claude -p` session, instructed to modify ONLY `transactions.test.ts`. Preserved all 15 existing tests; added 5 new AC tests covering: AC1 (member role 403), AC2 (manager role 200 + `posted >= 1`), AC3 (admin role 200), AC4 (activity_log entries for flipped rows), AC6 (`GET /:id/history` returns non-`__deleted__` entries for `transaction.gl_post`). Also added a seeded `TXN-AP-002` AP PENDING_PAID row with `glPosted: false` for AC2's determinism, per the regroomed spec. Stage 1 outcome: 16 pass / 4 fail — the expected red state (AC3 trivially passes because admin has no gate to fail against yet; the other four ACs drive the real red). `transactions.ts` untouched.

**Stage 2 (build).** Separate fresh session, instructed to modify ONLY `transactions.ts` against the frozen tests. Outcome: 20 pass / 0 fail, 66 expect() calls. Code changes followed the spec's §Skeleton Position line-for-line:

- `requireRole` imported and inserted as positional middleware (matches `clients.ts:31` precedent — which was non-blocking suggestion in the review's notes, not the spec's primary citation; the executor synthesized the tighter pattern from the reviewer's observation)
...
**No spec ambiguities required judgment** (unlike FRD-157 where AC3's status code was open). The spec's §Blast-Radius Manifest specified concrete line targets; the `field: 'gl_posted'` shape was suggested in the spec's AC6 verification text; the batch-level timestamp reuse was the only minor convergence call and it's defensible on semantics.

**Implication:** build validation generalizes beyond the simplest Bug shape. A Feature with new middleware usage + new activity-log attribution pattern + a per-surface mapper fix built cleanly in one pass from the spec + frozen oracle. The grooming-layer generator-evaluator pattern produces specs that are empirically trustworthy for non-trivial downstream implementation, and the reviewer's non-blocking observations are usable by Stage 2 even without an explicit "act on notes" instruction.
...
**Stage 1 (coverage gap-fill, all green against unmodified source).** Executor added 6 regression tests to `upload-idempotency.test.ts` locking in the spec's 7 observable invariants: bank mismatch / in-progress (existing tests only covered the extract side), three failed-400 persistence cases (invariant 6, missing entirely), and the soft-rollout warn-log shape (invariant 7). Final state: 12 pass / 0 fail / 55 expect() calls. Full backend suite: 240 pass / 7 fail / 1 skip — the 7 failures pre-existing at SHA 2edac08, unrelated to REF-001.
...
**Implication:** build validation now demonstrated across all three major type shapes. The grooming-layer generator-evaluator pattern produces specs empirically trustworthy for non-trivial downstream implementation, across Bug (27 assertions), Feature (66 assertions), and Refactor (55 assertions) with end-to-end Stage 1/Stage 2 discipline preserved.

## Spec-reviewer Groundedness has a blind spot for Refactor specs (0.3, REF-001)

The Stage 1 executor surfaced a finding the reviewer's three-check rubric does not currently catch:

The spec's Preservation Proof table listed 6 existing tests as "covering" 4 of 7 observable invariants. Those 6 tests *existed*, *imported the target routes*, and *looked structurally correct*. The spec-reviewer's Check 2 (Groundedness) verified exactly those three things. It approved the spec.
...
**Calibration candidate for the spec-reviewer (not actioned in 0.3):** for Refactor specs with `preservation-status: needs-coverage-first`, Groundedness should require running the cited tests at baseline — or equivalently, the pre-groomer should assert which cited tests currently pass (and only list those), forcing failing-at-baseline tests into the coverage-gap list where Stage 1 must repair or add them.
...
- Pre-groomer returned `design-status: need-info` and refused to produce a unified spec.
...
- Spec-reviewer APPROVED the need-info spec without inventing a middle label. Reviewer reasoned: "Mechanically applying the standard Feature schema would force the pre-groomer to author answers to open questions that belong to the customer/operator — the opposite of the scope-discipline rule."

**Implication:** deliver now has three branch states for approved specs:

- `verdict: APPROVED` + implementable spec → proceed to oracle writing.
- `verdict: APPROVED` + `design-status: need-info` → operator splits the issue per proposed decomposition, re-dispatches pre-groomer on each sub-issue; do NOT execute from the parent spec.
- `verdict: REJECTED` → pre-groomer regrooms.

This terminal state is emergent, not specced. Worth capturing in the spec-reviewer spec if a second data point confirms usefulness.
...
**Executor judgment overrode spec example.** Spec's concrete example used `colors`/`typography` as demo tab labels — those would have collided with the test's `getDemoTablist` filter excluding `Colors|Typography|Components|Motion`. Stage 2 correctly deviated. Healthy: examples are illustrative, not prescriptive.
...
2. ~~Does explicit decomposition-reflex guidance materially improve pre-groomer scoping on cross-cutting Features?~~ **Answered by 0.3.** Pre-groomer returned `design-status: need-info` + proposed decomposition on FRD-143 where 0.1 had produced a unified over-scoped spec.
3. ~~Does build validation generalize from a Bug to a Feature, especially FRD-122?~~ **Answered by 0.2.** FRD-122 → 20/20 tests green, 66 assertions, single-pass Stage 2, zero spec ambiguities.
...
7. *From 0.3 D05 finding.* Does the spec-reviewer's Groundedness check need to verify that cited coverage tests actually pass at baseline, not just that they exist? *Single data point from REF-001; needs second confirmation before spec revision.*
8. *From 0.3 companion.* Should `design-status: need-info` + `verdict: APPROVED` be explicitly codified as a terminal state in the spec-reviewer spec, with a named next-action for the operator? *Two data points now — FRD-143 (0.3) + FRD-161 (0.4). Pattern stable; worth codifying.*
...
10. *New — from 0.5 D05 finding.* Third occurrence of pre-groomer emitting an invented status value (`design-status: drafted`) with reviewer accepting. This is recurrent enough to warrant systemic fix: enumerate valid frontmatter values (`design-status`, `reproduction-status`, `preservation-status`) and have spec-reviewer Check 1 reject non-enumerated values. Calibration candidate.
...
12. *New — from 0.5.* Does the deliver pattern extend to a UI issue whose correctness DEPENDS on operator-committed design direction (FRD-161a-style decision spec)? 0.5's FRD-162 was deliberately direction-agnostic; harder cases remain untested.

...
Stable knowledge (how autoship works in general) lives in the controller agent file (`.claude/agents/autoship-controller.md`). This file is per-repo: what THIS repo's deliver runs should do.
...
- **Invocation:** from the testbed root, `claude --agent autoship-controller -p "deliver"` to resume in-flight work, or `claude --agent autoship-controller -p "deliver FRD-162"` to restrict to one issue.

...
- **Building** — the oracle writer creates the evidence contract. The implementation executor writes the code, and it is **forbidden** from modifying frozen evidence files. The oracle-freeze plays the role of the separate reviewer; evidence mutation becomes a visible signal that something's wrong.
...
Build PRs are review packets, not just implementation diffs. A completed build PR includes a `Human Review Checklist` with issue-specific inspection points, validation gaps or manual checks, and risk areas. For UI/frontend changes, the handoff also includes `Preview Evidence` in the PR body and, when posting is enabled, the matching Linear build-complete comment. The default evidence path is Playwright CLI capture against a local dev server when the app can run locally; hosted preview evidence is a fallback when local capture is not feasible. The controller reconciles PR comments/status checks after PR creation, links hosted previews when available, includes screenshot or screen-recording links/paths, and states the exact blocker when visual evidence was not captured. Missing visual evidence does not imply that no preview exists.
...
- **Preservation Proof** — executable behavior evidence that covers the refactor target: command, files/fixtures, and claims covered. Existing behavior tests cited here are oracle obligations, not optional human-review advice. Identified gaps name specific regression tests the spec commits to adding BEFORE the refactor lands.
...
| **Oracle writer** | Designs the **evidence contract** that will judge the code from the approved spec. | Tests, fixtures, and test harness only. The spec is read-only; production code is read-only. Owns `oracle/result.md`. | Implementation executor (evidence passes once code is written, or does not) and the human reviewer on the PR. |

The oracle is adaptive but fail-closed. It searches the repo for existing tests, fixtures, helpers, seed scripts, sample files, and commands before creating new evidence. It may generate minimal realistic fixtures when grounded, and it may use mocks only when the claim is specifically about UI wiring or real integration is not feasible. Typecheck, lint, route generation, and grep are supporting evidence, not behavioral proof. If the oracle cannot honestly prove the material claims, it returns `oracle-insufficient-evidence` and the controller parks the issue instead of dispatching implementation.

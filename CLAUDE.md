...
- **Deliver state-as-baton.** Deliver mirrors handoffs to Linear only when posting is enabled. Local runs are local-first; pass `--post` to emit state changes + @mention comments, and remote runners may pass `--post` as policy. Recommended remote states are `Ready to Groom` (agent may analyze and, if clear, build), `Breakdown Proposed` (review the breakdown PR), `Breakdown Approved` (create child issues and start dependency-free slices), and `Needs Attention` (human unblock). `Spec Ready` remains optional supervised compatibility, not the default remote path. State and label are orthogonal axes (state = baton; label = artifact kind). State transitions are best-effort: if a target state is missing, the comment still posts. Canonical Linear policy table lives in `.claude/agents/autoship-controller.md § Linear policy`.

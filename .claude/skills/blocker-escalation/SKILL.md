...
Convert an unrecoverable state into a structured blocker report so the controller (or a human) can make the smallest decision that unblocks the loop. The skill stops the executor from guessing when it should be escalating.
...
- The controller has already approved a retry path for this exact failure class.
...
- **Why each attempt was insufficient** — the diagnostic pattern across attempts, if any. This is what lets the controller decide whether the blocker is the right escalation or whether one more slice is warranted.
...
These rules are what make the report usable to the controller. Violating them produces a blocker that wastes a triage pass without moving the loop forward.

1. **Evidence, not summary.** The controller routes on evidence. A report that says "the tests failed" produces a triage round asking for the test output. Paste the output in the first report.
...
3. **Don't touch protected surfaces during escalation.** If you are writing a blocker because a protected surface needs changing, the blocker's whole point is to propose that change for the controller to authorize — not to sneak the change in while you're escalating.
...
| "The blocker is probably obvious to the user" | The blocker must be explicit and actionable without conversation. The controller may be acting without a human in the loop. |
...
| "I'll file the report and also try the fix anyway" | The fix-while-escalating pattern corrupts "what was tried." File, then wait for the decision. If the controller authorizes the fix, do it then. |
...
A report that passes the lint is not automatically a good report — but a report that fails the lint is definitely not ready to file. Judgment criteria (is the decision really the smallest one? is the evidence actually localized?) live with the controller reviewer, not with the script.

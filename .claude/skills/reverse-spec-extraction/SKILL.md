...
Full schemas, field tables, and runtime-neutral examples live in `references/schemas.md`. Read it at synthesis time — you don't need it on every probe.

Summary of the required information categories:

- **Endpoint** (`api-spec.declared.json`, `api-spec.observed.json`, merged `api-spec.json`): `method`, `path_template`, `source_location`, `handler_name`, request + response schemas (field-by-field, including error cases), `side_effects`, optional `notes`. Merged artifact adds `presence`, `schema_agreement`, `schema_agreement_reason`, and nested `declared: {...}` / `observed: {...}` sub-objects that preserve every source field.
- **Entity** (`data-model.declared.json`, `data-model.actual.json`, merged `data-model.json`): `name`, `source`, `fields`, optional `relationships`, optional `notes`. Merged artifact adds `sources`, `authoritative_source`, `drift`, and nested `declared: {...}` / `actual: {...}` sub-objects.

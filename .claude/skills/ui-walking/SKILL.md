---
name: ui-walking
description: Use when an autoship worker must drive a running application's UI to produce verification evidence against an oracle's UI journeys. The skill governs how to launch the app safely, walk a journey as the oracle declares it, observe the resulting state, and capture screenshots + transcript + structured observed-state JSON the controller can mechanically compare against expected state. Reach for it whenever an oracle declares a `ui_journeys` block and a worker is being asked to execute it. Do not use it to invent journeys, to judge UX freely, or to author the oracle itself.
---
...
Drive a running application's UI to produce **oracle-anchored verification evidence**. The oracle declares the journeys and the expected observable state at each step; the walker executes the journey and records what the app actually showed. The walker does not decide whether the app is correct — the oracle (via the controller's verification step) does.
...
- `verdict.json` — machine-readable observed state per journey, ready for the controller to compare against oracle expectations

Everything else in this skill exists to keep that evidence pack honest.

## Posture

**You are the executor, not the judge.** The oracle declares correctness; you record reality. When the app does something surprising, your job is to capture it as observed state and let the controller decide whether that observed state matches the expected state. Do not silently retry, do not "fix" by clicking around, do not infer intent. Observed reality is the artifact.
...
- The oracle for a slice declares a `ui_journeys` block (or equivalent browser-check evidence) and the controller has dispatched you to execute it.
...
- No oracle exists yet — the walker is the runtime sibling of `deliver-implementation`, not a substitute for `deliver-oracle-writer`.
...
The walker does NOT compute pass/fail. It records observed state. The controller's verification step compares `verdict.json[<journey>].observed` against the oracle's `expected` for that journey.

## Discipline

These rules are what make the evidence pack usable to the controller. Violating them produces evidence that wastes a verification pass or, worse, lets a regression ship.

1. **Observe, don't judge.** Do not write `status: pass` into `verdict.json` based on your interpretation. Write the raw observed values. The controller computes the verdict.
...
3. **Capture console + network errors always, even when the journey "succeeds."** A journey that succeeds *with* a 500 in the network log is evidence the controller wants to see. Silent suppression is a bug.
...
5. **Do not modify the application or test data to make a journey pass.** That is `deliver-implementation`'s scope, not yours. If the journey cannot be executed because the app is broken, that *is* the verdict — record it and stop.
...
| "The expected text is missing but the page looks right" | Record observed state. The controller decides. Your "looks right" is not evidence. |
| "There's a console error but the flow finished" | Always record the console error. The controller may treat it as a failure even if the flow finished. |
...
- `.claude/skills/blocker-escalation/` — when the walker cannot proceed

## verdict.json shape (v1 reference)

The controller reads this. Be precise.

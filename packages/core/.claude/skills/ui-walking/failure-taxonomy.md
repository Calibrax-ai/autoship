...
- **Downstream meaning** (controller's job, not walker's): probably `verification-failed`, route fix to `deliver-implementation`.
...
- **Downstream meaning**: controller compares observed vs. expected and decides whether `verification-failed` (implementation) or `oracle-rewrite-needed` (oracle).
...
- **Downstream meaning**: controller decides whether ambient errors are tolerable or escalating.
...
- **Downstream meaning**: controller halts, operator decides.
...
- **Downstream meaning**: controller marks the journey `flaky` and routes the next dispatch to `deliver-oracle-writer` (timing/entropy fix), matching the existing flakiness pattern in `verification`.
...
- **Downstream meaning**: infrastructure issue; controller routes to operator.
...
The categories exist so the controller can mechanically route follow-up work without re-reading the entire transcript. They are NOT verdicts; they are *failure shapes*. The verdict (pass/fail/insufficient) is computed by the controller from `observed` vs. `expected` plus the failure_kind hint.

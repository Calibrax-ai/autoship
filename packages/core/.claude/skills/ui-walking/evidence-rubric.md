...
Paraphrase is the controller's job, not the walker's.
...
The report is a human-readable view onto the same data the controller machine-reads. They must agree. If `verdict.json` says one thing and `report.md` says another, the evidence pack is corrupt and the journey must be re-executed.
...
- A pass verdict from the walker itself — the walker does not issue verdicts. The controller does.
